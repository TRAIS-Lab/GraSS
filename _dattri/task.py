...
class AttributionTask:
    ...
    def _load_checkpoints(self, index: int, drop_out: bool) -> None:
        ...
        if not drop_out:
            ...
            self.model.train() # only for MLP (otherwise it'll affect other stuffs)
    ...
    def get_param(
        ...
        Raises:
            ValueError: If the length of param_layer_map is not the same as the length
                of named_parameters
        """
        self._load_checkpoints(index, drop_out)
