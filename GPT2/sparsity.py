...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="GD-default",
        help="Specify which mode we want to run the sparsity inspection. Available options: GIP-default, GD-default, and TRAK-default",
    )
    parser.add_argument(
        "--device",
        type=int,
...
def main():
    ...
    if tda_method == "GIP":
        if tda_mode == "default":
            ...
            raise ValueError("Invalid mode type for sparsity evaluation. Choose from 'default', 'one_run'.")
    elif tda_method == "GD":
        if tda_mode == "default":
                train_batch_size = 4
                test_batch_size = 4
        else:
            raise ValueError("Invalid model type for vanilla Grad-Dot. Choose from 'default'.")
    ...
    thresholds = [0.0, 0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1]
    ...
    if tda_method == "GIP":
        ...
        sparsity = attributor.sparsity(train_dataloader, thresholds)
    ...
    filename_parts = [f"{tda_method}-{tda_mode}", "sparsity"]
