...
def SJLT_batch(batch_vec, proj_dim, rand_indices, rand_signs, c=2, blow_up=1):
    ...
    # scaled_vals_flat = scaled_vals.flatten()
    # output_indices_flat = output_indices.flatten()
    # batch_idx_repeated = batch_idx.repeat_interleave(c)

    # batch_vec_p.view(-1).index_add_(
    #     0,
    #     batch_idx_repeated * (proj_dim * blow_up) + output_indices_flat,
    #     scaled_vals_flat
    # )
    ...
    # use expand to create indices
    # batch_idx_repeated = batch_idx.unsqueeze(1).expand(-1, c).flatten()
    # final_indices = (batch_idx_repeated * (proj_dim * blow_up) + output_indices).flatten()
