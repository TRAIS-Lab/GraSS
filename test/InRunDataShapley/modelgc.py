...
# class GCLayerNorm(nn.Module):
#     def __init__(self, normalized_shape, eps=1e-5):
#         super(GCLayerNorm, self).__init__()
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attn = GCLinear(config.n_embd, 3 * config.n_embd, bias=config.bias)
        # output projection
        self.c_proj = GCLinear(config.n_embd, config.n_embd, bias=config.bias)
        # regularization
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.c_fc    = GCLinear(config.n_embd, 4 * config.n_embd, bias=config.bias)
        self.gelu    = nn.GELU()
        self.c_proj  = GCLinear(4 * config.n_embd, config.n_embd, bias=config.bias)
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None, return_per_token=False, example_weights=None):
        ...
        if targets is not None:
            # if we are given some desired targets also calculate the loss
            # reshape logits and targets to compute per-token loss
            targets = targets.view(-1)
            
            if return_per_token:
                # compute per-token loss without reduction
                loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets, ignore_index=-1, reduction='none')
                # reshape loss to match the original batch and sequence length
                loss = loss.view(b, t)
            else:
                # compute average loss across all tokens
                loss = F.cross_entropy(logits.view(-1, logits.size(-1)), targets, ignore_index=-1, reduction='none')

                loss = loss.view(b, t).mean(dim=1)  # mean over sequence length

                if example_weights is not None:
                    example_weights = torch.from_numpy(example_weights).float().to(device)
                    assert example_weights.size(0) == b, "example_weights must have the same batch size as input"
                    # apply example weights
                    loss = (loss * example_weights).sum()  # weighted average loss
                else:
                    loss = loss.mean()  # average loss across batch
        else:
            # inference-time mini-optimization: only forward the lm_head on the very last position
            logits = self.lm_head(x[:, [-1], :])  # note: using list [-1] to preserve the time dim
            loss = None
    ...
    def configure_optimizers(self, weight_decay, learning_rate, betas, device_type, sgd=False):
